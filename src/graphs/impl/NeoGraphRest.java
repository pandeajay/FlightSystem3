...
public class NeoGraphRest implements Graph{
	...
	public static void main(String[] args){
		...
		business.impl.NodeImpl node1 = new business.impl.NodeImpl("1", toListWithWeight);
		map.clear();
		toListWithWeight.clear();

		long id = neoGraph.addNode(node1);
	...
	@Override
	public long addNode(business.Node node) {
		...
		NeoGraphRest.nodeIAndNeoId.put(node.getNodeId(), "" + myNode.getId());
		return myNode.getId();
	 ...
	 */
	@Override
	public long addEdge(business.Node node) {
		if(!validateNode(node)){
			return 0;
		}
			long fromId = 0 ;
			try{
				fromId = Long.parseLong(NeoGraphRest.nodeIAndNeoId.get(node.getNodeId()));
			}catch(Exception ex){
		...
		while(it.hasNext()){
			...
			Relationship edge = fromNode.createRelationshipTo(toNode, RelationTypes.RelTypes.TravellingTo);
			double weight = 0.0;
			try{
	...
	public double getShortestPathWeight(String from, String to) {
		...
		PathFinder<WeightedPath> finder = GraphAlgoFactory.dijkstra(
				PathExpanders.forTypeAndDirection( RelationTypes.RelTypes.TravellingTo, Direction.OUTGOING ), "weight" );
	 ...
	 */
	@Override
	public void addEdges(List<business.Node> nodes) {
		if(nodes == null || nodes.size() == 0){
	...
	@Override
	public void deleteNodes(List<business.Node> nodes) {
		if(nodes == null || nodes.size() == 0){
	 ...
	 * @return
	 */
	boolean validateNode(business.Node node){
		if(node == null){
